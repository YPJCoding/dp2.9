...
- Added `tools/check_df_game_r_start.py` to distinguish pending, patched, and mixed `df_game_r.js start()` states.
...
- Continued Frida migration hardening:
  - `script/js/startup_helpers.js` now caches loaded modules to avoid repeated `dp_load` of the same module during one startup flow, and resolves startup functions through `globalThis`, current context, and `eval(functionName)` fallback before reporting a missing function.
...
- Recorded that remote `df_game_r.js` on `refactor/dp2-9-base` is still in old `start()` pending state; the local patch described by `docs/DF_GAME_R_START_PATCH.md` has not been committed to the branch yet.
...
- `df_game_r.js start()` remote state should be checked with `python3 tools/check_df_game_r_start.py`; current remote branch was confirmed pending before committing the checker.
