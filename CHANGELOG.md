...
- Continued Frida migration hardening:
  ...
  - `script/js/village_attack_db.js` now provides the migrated `event_villageattack_save_to_db()` / `event_villageattack_load_from_db()` helpers with DB-ready and JSON parse guards.
  - `script/js/village_attack_hook.js` now also provides the migrated `VillageAttackedRewardSendReward(user)` callback, preserving the old reward mail mapping.
  - `df_game_r.js` no longer keeps the village-attack DB helper bodies or the reward callback body; native declarations and common mail/MySQL/API helpers remain in place for migrated modules.
  - Added/verified patch and check scripts for the village-attack DB helper and reward callback cleanup.
...
- Recorded that `df_game_r.js start()` has been switched to the centralized `startup_helpers` / `startup_modules` path on `refactor/dp2-9-base`.
...
- Fixed `tools/patch_df_game_r_village_attack_db.py` function-block boundary detection so the replacement marker is not removed while deleting adjacent helper functions.
...
- Removed the migrated `enable_drop_use_luck_point()` helper body from `df_game_r.js`; `script/js/luck_point_drop.js` provides `startLuckPointDrop()` and retains the old `enable_drop_use_luck_point()` alias.
...
- `df_game_r.js start()` should stay `patched` when checked with `python3 tools/check_df_game_r_start.py`; it now loads `startup_helpers` / `startup_modules` and calls `startMigratedModules(cfg)`.
