#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated drop-announce helper from df_game_r.js.

The active implementation is now provided by script/js/drop_announce.js.
...
- hook_history_log
- api_GameWorld_SendNotiPacketMessage / api_recharge_cash_cera helpers
- CDataManager / CItem helper declarations

Usage from repo root:
    python3 tools/patch_df_game_r_drop_announce.py
    python3 tools/check_df_game_r_drop_announce.py
"""
...
REPLACEMENT_HEADER = """// 掉落公告/奖励函数已迁移到 script/js/drop_announce.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[drop_announce_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if found is None:
            print("[drop_announce_patch] function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[drop_announce_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[drop_announce_patch] patched df_game_r.js")
    return 0
