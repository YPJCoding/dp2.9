#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated village-attack DB helpers from df_game_r.js.

The DB helpers are now provided by script/js/village_attack_db.js.
...
Usage from repo root:
    python3 tools/patch_df_game_r_village_attack_db.py
    python3 tools/check_df_game_r_village_attack_db.py
"""
...
def main() -> int:
    if not TARGET.exists():
        print(f"[village_attack_db_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if first is None:
            print("[village_attack_db_patch] first db function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[village_attack_db_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[village_attack_db_patch] patched df_game_r.js")
    return 0
