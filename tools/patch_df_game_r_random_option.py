#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated random-option helpers from df_game_r.js.

The helpers are now provided by script/js/random_option.js.

This removes only:

- _boost_random_option_equ
- boost_random_option_equ
- change_random_option_inherit
- auto_unseal_random_option_equipment

It intentionally does not remove:

- random_option_CRandomOptionItemHandle_give_option native declaration
- history log comments / call sites
- unrelated inventory and item helpers

Usage from repo root:
    python3 tools/patch_df_game_r_random_option.py
    python3 tools/check_df_game_r_random_option.py
"""
...
REPLACEMENT_HEADER = """// 随机属性相关函数已迁移到 script/js/random_option.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[random_option_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if first is None:
            print("[random_option_patch] first function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[random_option_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[random_option_patch] patched df_game_r.js")
    return 0
