#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated hook_VillageAttack from df_game_r.js.

The hook body is now provided by script/js/village_attack_hook.js.
...
- reward mail / cera settlement helpers
- any native declarations

Usage from repo root:
    python3 tools/patch_df_game_r_village_attack_hook.py
    python3 tools/check_df_game_r_village_attack_hook.py
"""
...
FUNCTION_NAME = "hook_VillageAttack"
REPLACEMENT_HEADER = """// 怪物攻城 hook 已迁移到 script/js/village_attack_hook.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[village_attack_hook_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if found is None:
            print("[village_attack_hook_patch] hook function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[village_attack_hook_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[village_attack_hook_patch] patched df_game_r.js")
    return 0
