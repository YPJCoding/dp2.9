#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated batch item notification helpers from df_game_r.js.

The helpers are now provided by script/js/batch_item_notify.js.

This removes only:

- api_CUser_Add_Item_list
- SendItemWindowNotification

Usage from repo root:
    python3 tools/patch_df_game_r_batch_item_notify.py
    python3 tools/check_df_game_r_batch_item_notify.py
"""
...
REPLACEMENT_HEADER = """// 批量物品 UI 通知函数已迁移到 script/js/batch_item_notify.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[batch_item_notify_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if first is None:
            print("[batch_item_notify_patch] first function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[batch_item_notify_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[batch_item_notify_patch] patched df_game_r.js")
    return 0
