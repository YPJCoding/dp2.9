#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated emblem-fix helper from df_game_r.js.

The hook implementation is now provided by script/js/emblem_fix.js.

This removes only:

- fix_use_emblem

It intentionally keeps shared helpers used by the migrated module:

- api_get_avatar_ui_id
- api_set_JewelSocketData

Usage from repo root:
    python3 tools/patch_df_game_r_emblem_fix.py
    python3 tools/check_df_game_r_emblem_fix.py
"""
...
FUNCTION_NAME = "fix_use_emblem"
REPLACEMENT_HEADER = """// 时装镶嵌修复函数已迁移到 script/js/emblem_fix.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[emblem_fix_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if found is None:
            print("[emblem_fix_patch] function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[emblem_fix_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[emblem_fix_patch] patched df_game_r.js")
    return 0
