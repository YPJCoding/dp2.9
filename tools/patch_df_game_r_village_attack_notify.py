#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated village-attack notify helpers from df_game_r.js.

This removes only functions now provided by script/js/village_attack_notify.js:

- gameworld_update_villageattack_score
- notify_villageattack_score

It intentionally does not remove hooks, PT phase transitions, rewards, DB save/load,
or event settlement.

Usage from repo root:
    python3 tools/patch_df_game_r_village_attack_notify.py
    python3 tools/check_df_game_r_village_attack_notify.py
"""
...
REPLACEMENT_HEADER = """// 怪物攻城 UI/进度通知函数已迁移到 script/js/village_attack_notify.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[village_attack_notify_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if first is None:
            print("[village_attack_notify_patch] first migrated function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[village_attack_notify_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[village_attack_notify_patch] patched df_game_r.js")
    return 0
