#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Remove migrated history-log helper from df_game_r.js.

The active implementation is now provided by script/js/history_log.js.
...
- UserUseItemEvent (already handled by user_use_item_event.js)
- CDataManager / CItem / Interceptor helper declarations

Usage from repo root:
    python3 tools/patch_df_game_r_history_log.py
    python3 tools/check_df_game_r_history_log.py
"""
...
FUNCTION_NAME = "hook_history_log"
REPLACEMENT_HEADER = """// 历史日志追踪函数已迁移到 script/js/history_log.js。
...
def main() -> int:
    if not TARGET.exists():
        print(f"[history_log_patch] missing {TARGET}", file=sys.stderr)
        return 2
    ...
    if REPLACEMENT_HEADER not in text:
        ...
        if found is None:
            print("[history_log_patch] function not found", file=sys.stderr)
            return 2
    ...
    if text == original:
        print("[history_log_patch] no changes")
        return 0

    TARGET.write_text(text, encoding="utf-8")
    print("[history_log_patch] patched df_game_r.js")
    return 0
