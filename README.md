...
6. 安全可部署版优先保证启动链路、默认安全行为和低/中风险功能可用；完全功能恢复版后续逐项验证。
...
- [x] legacy_patches 默认关闭与子功能开启路径已通过测试服验证
- [x] hot_reload config-only 主路径已通过测试服验证
- [x] finish_back_home 主要模式与装备品质过滤已通过测试服验证
...
- [ ] 添加并验证觉醒、转职、跨界、SQL/删除/shell 类正式道具行为
...
- [x] `online.lua`：在线玩家表、上线/下线记录。
- [x] `broadcast.lua`：全服消息，带频率限制。
- [x] `gm_permissions.lua`：GM 权限判断基础模块。
- [x] `hot_reload.lua`：config-only 热加载。
- [x] `finish_back_home.lua`：翻牌回城 / 自动处理 / 随机点券，主要模式已验证。
- [x] `legacy_patches.lua`：旧入口补丁，默认关闭与子功能开启路径已验证。
