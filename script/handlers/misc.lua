...
local function reject_sql_disabled(user, item_id, ctx, message)
    local dpx = ctx.dpx
...
function M.register(item_handler, ctx)
    ...
    item_handler[2023458801] = function(user, item_id)
        if not is_sql_handler_enabled(ctx) then
            return reject_sql_disabled(user, item_id, ctx, "注意： 角色出战功能未开启！")
        end
    ...
    item_handler[2023458803] = function(user, item_id)
        if not is_sql_handler_enabled(ctx) then
            return reject_sql_disabled(user, item_id, ctx, "注意： 装备设计图熟练度功能未开启！")
        end
